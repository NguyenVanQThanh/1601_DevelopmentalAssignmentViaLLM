...
//                a way to start new conversations. It also displays a disclaimer.
...
function ChatbotPage({ initialMessage, sessionId, accessToken, onInitialMessageShown }) {
  ...
  // isLoading: boolean - State to indicate if an API call is in progress.
  const [isLoading, setIsLoading] = useState(false);
  // error: Optional[string] - State to store and display API call error messages.
  const [error, setError] = useState(null);
  // chatBodyRef: React.RefObject - Ref for the chat body div for scrolling.
  const chatBodyRef = useRef(null);
  // isMounted: React.RefObject - Ref to track component mount status to prevent state updates on unmounted components.
  const isMounted = useRef(true);

  // API_BASE_URL: string - Base URL for backend API calls.
  const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';

  // disclaimerText: string - The disclaimer text to be displayed.
  const disclaimerText = "Lưu ý: Thông tin từ AI chỉ mang tính tham khảo, không thay thế cho chẩn đoán hoặc tư vấn y tế từ bác sĩ chuyên khoa. Phụ huynh nên đưa trẻ đến gặp bác sĩ nhi khoa để được kiểm tra và tư vấn cụ thể.";

  // Function Name: scrollToBottom
  // Functionality: Scrolls the chat body to the bottom.
  // Input: None.
  // Output: None.
  ...
  // Effect for component mount/unmount tracking.
  useEffect(() => {
  ...
  // Functionality: Fetches chat history from the backend if session is active and no messages are present.
  // Input: None.
  ...
  // Effect for handling initial message and fetching history.
  useEffect(() => {
    if (isMounted.current) {
      if (initialMessage && messages.length === 0) {
        setMessages([{ sender: 'bot', text: initialMessage }]);
        if (onInitialMessageShown) onInitialMessageShown();
  ...
  // Functionality: Clears backend chat history for the session and resets local messages.
  // Input: None.
  // Output: None (updates 'messages' state, calls backend).
  ...
  // Functionality: Sends user's message to the backend /ask endpoint and updates chat.
