...
% ---------- KEYWORDS ----------
\begin{IEEEkeywords}
Autism Spectrum Disorder, Machine Learning, Psychological Medical Records, SMOTE-IPF, OLA, Screening Tool
\end{IEEEkeywords}
...
Consequently, the Overall Local Accuracy (OLA) model also achieves very good performance, showcasing its adaptability to the data and surpassing both XGBoost and CatBoost in overall outcomes.
...
a1) Metrics Cross Validation: \\
- The Decision Tree model continues to yield moderately low results, performing the least effectively among the evaluated models.

- CatBoost outperforms Decision Tree but exhibits suboptimal performance due to the instability of the dataset.

- XGBoost delivers better results than CatBoost, though with a slight deviation of approximately 0.01.

- The Overall Local Accuracy (OLA) model achieves significantly superior evaluation results, demonstrating notable flexibility in adapting to the unstable clinic dataset, surpassing both XGBoost and CatBoost.

- The enhanced OLA framework shows no significant improvement in evaluation metrics, with results remaining comparable to the traditional OLA or showing only a minor difference.

Consequently, OLA emerges as the most suitable model for a dataset lacking stability and clear data trends, though the potential for column bias in this unstable dataset may contribute to the observed inconsistencies.
...
Consequently, the dataset is predominantly biased toward the "TiepXucMat" column, necessitating enhancements to the adaptable OLA model. These improvements should aim to achieve a more uniform and balanced differentiation across columns, ensuring that the screening process does not miss positive cases.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.45\linewidth}
