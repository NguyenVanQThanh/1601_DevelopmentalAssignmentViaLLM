...
% ---------- TITLE ----------
\title{A Machine Learning-Based Tool for Autism Screening Using Psychological Medical Records}

\author{Hao Nguyen Thi Bich\inst{1} \and
Thanh Nguyen Van Quoc\inst{2} \and
Thuan Nguyen Dinh\inst{3} \and
Nhut Nguyen Minh\inst{4}}
\authorrunning{Nguyen T.B. Hao, Nguyen V.Q. Thanh, Nguyen D. Thuan, Nguyen M. Nhut}
...
\email{\{21521447@gm.uit.edu.vn, 21522049@gm.uit.edu.vn, thuannd@uit.edu.vn, nhutnm.17@grad.uit.edu.vn\}}}

\maketitle

% ---------- ABSTRACT ----------
\begin{abstract}
Autism Spectrum Disorder (ASD) is a neurodevelopmental condition that impacts social interaction and behavior, where early detection is crucial for optimizing intervention outcomes. 
...
Autism Spectrum Disorder (ASD) is a neurodevelopmental condition marked by difficulties in social interaction, communication, and repetitive behaviors~\cite{b1}. Early detection is vital for effective interventions, yet diagnosis becomes more challenging with age due to symptom overlap with other disorders. In the U.S., ASD prevalence has increased from 1 in 150 (2000) to 1 in 36 (2023)~\cite{b2}; in Vietnam, estimates suggest 1 in 68 children, though official data remains limited.

Conventional tools like M-CHAT-R/F and ADOS require expert administration and are less accessible in low-resource settings such as Vietnam~\cite{b3}. The absence of definitive medical tests further complicates diagnosis, relying on subjective behavioral assessments. Recent advances in AI and ML enable automated ASD screening using electronic medical records (EMRs), though challenges like imbalanced data and feature bias remain~\cite{b4}. While models like Decision Trees, XGBoost, and CatBoost perform well in controlled environments, they often lack generalizability in real-world data.
...
\begin{itemize}
\item \textbf{Step 1:} Raw data was extracted from PDF documents and categorized by diagnosis, selecting over 800 ASD-related and non-clinical cases.
\item \textbf{Step 2:} Incomplete or low-quality records were excluded, yielding 600 usable samples. Key behavioral features - such as eye contact~\cite{b11}, pointing~\cite{b12}, response to name~\cite{b13}, joint attention~\cite{b14}, imitation~\cite{b15}, and symbolic play~\cite{b16} - were labeled under expert guidance. All personal identifiers were encoded to preserve privacy.
\item \textbf{Step 3:} The final dataset contains 594 structured records, with 36 features (raw + labeled). The dataset is imbalanced, with 93% of samples diagnosed with ASD or under ASD tracking, and a dominant clinical evaluation feature contributing to potential feature bias.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Feature 1} & \textbf{Feature 2} & \textbf{Feature 3} \\ \midrule
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/processASD.drawio.png} 
...
All personally-identifiable information was either encrypted or removed before model training.

\subsubsection{\textbf{Step 2:} Redundant Column Removal}
Columns lacking analytical value-such as purely administrative metadata-were eliminated.

\subsubsection{\textbf{Step 3:} Missing Value Imputation}
Numerical attributes were imputed with the median, while categorical attributes were filled with the mode; domain-expert psychology rules guided these choices where appropriate.
...
% \vspace{0.5em}
SMOTE-IPF~\cite{b6} was selected as the main balancing method due to its effectiveness in handling class imbalance and filtering out noisy data. Unlike SMOTE-KNN~\cite{b7}, which imputes missing values but lacks noise filtering, SMOTE-IPF reduces the risk of overfitting.

To improve model training, unsafe ASD samples were further resampled using KNN, resulting in three datasets:
\begin{itemize}
\item A0: After SMOTE-IPF.
\item A1: Fully amplified unsafe ASD samples.
\item A2: Selective amplification based on neighbor voting.
\end{itemize}

These datasets, exported for training, showed that SMOTE-IPF offered the best trade-off between balance and robustness.

\subsection{Evaluate Models}
\subsubsection{a, Models}
...
\subsubsection{b, Evaluation Metrics and Validation}

\paragraph{\textbf{Evaluation Metrics}} Recall, Specificity, Precision, F1-score
...
\paragraph{\textbf{Validation Techniques}}
...
% \FloatBarrier
\subsection{Enhance with OLA Framework}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.5\linewidth]{images/structure_imp_ola.png} 
%     \caption{Framework OLA}
%     \label{fig:ola_pipeline}
% \end{figure}

\begin{itemize}
\item \textbf{Step 1:} Data Preprocessing~\\
\begin{itemize}
    \item Standardization with StandardScaler: Transform data to a standard normal distribution (mean=0, variance=1) to reduce the influence of features with large values (eg: TiepXucMat), ensuring efficient model performance.  
    \item Dimensionality Reduction with PCA (Principal Component Analysis): Reduce data dimensions, removing noise and bias from dominant features while lowering computational cost.  
\end{itemize}

\item \textbf{Step 2:} Choosing internal model\\
\begin{itemize}
    \item Model Group Creation: XGBClassifier, CatBoostClassifier, and RandomForestClassifier trained on PCA data, focusing on key features to minimize noise.  
    \item This diversity enhances flexibility, allowing OLA to select the most suitable model for each test sample.  
\end{itemize}

\item \textbf{Step 3:} Deploy OLA Model~\\
...
\paragraph{\textbf{a. Metrics Cross Validation:}} \\
% \begin{itemize}
...
\paragraph{\textbf{b. Chart Feature Importance Analysis:}}\\
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        \includegraphics[width=0.8\linewidth]{images/tree_cot.png}
        \caption{Decision Tree}
        \label{fig:x1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\linewidth}
        \includegraphics[width=0.8\linewidth]{images/xgb_cot.png}
        \caption{XGBoost}
        \label{fig:x2}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        \includegraphics[width=0.8\linewidth]{images/cat_cot.png}
        \caption{CatBoost}
        \label{fig:x3}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\linewidth}
        \includegraphics[width=0.8\linewidth]{images/ola_cot.png}
        \caption{OLA}
        \label{fig:x4}
    \end{subfigure}
...
\subsubsection{Private Dataset}\mbox{}\\
\paragraph{\textbf{a. Metrics Cross Validation:}} \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images/feature_enhanced_ola.png}
