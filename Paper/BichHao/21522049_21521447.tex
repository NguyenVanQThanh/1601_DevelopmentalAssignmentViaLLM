...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    \toprule
    \textbf{Feature 1} & \textbf{Feature 2} \\ \midrule
    Medical History & Turn-Taking Play \\
    Speech Delay & Imitation \\
    Behavioral Rigidity & Task Requesting \\
    Behavioral Isolation & Response to Name in Infants \\
    Sensorimotor Play & Toe Walking after 3 years \\
    Functional Play & Making a Point \\
    Combinatorial Play & Joint Attention \\
    Symbolic Play & Eye Contact \\
    Repetitive/Stereotyped Play & Social Communication Skills \\ 
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
    \item Standardization with StandardScaler: Transform data to a standard normal distribution (mean=0, variance=1) to reduce the influence of features with large values (e.g., TiepXucMat), ensuring efficient model performance.  
...
\item \textbf{Step 2:} Create PCA-based Group - Using PCA-reduced Data~\\
\begin{itemize}
    \item Model Group Creation: PCA-based group - XGBClassifier, CatBoostClassifier, and RandomForestClassifier trained on PCA data, focusing on key features to minimize noise.  
