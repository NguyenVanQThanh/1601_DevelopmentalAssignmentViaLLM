...
\caption{AI Chatbot Architecture Proposal}
\label{fig:architecture}
\end{figure}
The overall architecture of our system is based on the Retrieval-Augmented Generation (RAG) framework, combining a retriever and a large language model (LLM) for more accurate and context-aware responses. Figure~\ref{fig:architecture} illustrates the key components and their interactions.

The VectorDB FAISS component which is created from the BGE-M3 embeddings serves as the backbone for the retrieval process, allowing for efficient similarity searches over the document embeddings.

After fine-tuning, the LLM component receives a prompt that includes the user's question, relevant context, and screening test results (such as the ASQ-3), and then generates an appropriate response.
